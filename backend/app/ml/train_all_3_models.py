...
def train_and_eval_all_models():
    ...
    df["label"] = label_encoder.fit_transform(df["Cleaned_Category"])
    ...
    tfidf = TfidfVectorizer(max_features=30000, ngram_range=(1, 2), stop_words="english", sublinear_tf=True)
    ...
    lr = LogisticRegression(max_iter=1000, C=3.0, random_state=42)
    ...
    xgb = XGBClassifier(n_estimators=200, max_depth=7, learning_rate=0.08, subsample=0.85, random_state=42, n_jobs=-1)
